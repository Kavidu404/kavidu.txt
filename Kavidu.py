...
def banner():
	...
	ban='''
>>--------->
 __   ___       __  ___      ___  __     ________   ____  ____     
|/"| /  ")     /""\|"  \    /"  ||" \   |"      "\ ("  _||_ " |    
(: |/   /     /    \\   \  //  / ||  |  (.  ___  :)|   (  ) : |    
|    __/     /' /\  \\\  \/. ./  |:  |  |: \   ) ||(:  |  | . )    
(// _  \    //  __'  \\.    //   |.  |  (| (___\ || \\ \__/ //     
|: | \  \  /   /  \\  \\\   /    /\  |\ |:       :) /\\ __ //\     
(__|  \__)(___/    \___)\__/    (__\_|_)(________/ (__________)    
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='[KAVIDU] CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='[KAVIDU] OK'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					statusok = f'[bold green][•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green][KAVIDU] OK[/bold green]'))
					ok+=1
					break
